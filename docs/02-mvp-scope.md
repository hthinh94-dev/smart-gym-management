...
- **Hồ sơ thể trạng hội viên (Member Profile):**
  - *Mô tả:* Quản lý thông tin thể chất chi tiết gồm: họ tên, `dateOfBirth`, `gender`, chiều cao (`heightCm`), cân nặng (`weightKg`), tối đa hai mục tiêu (`fitnessGoals`: `MUSCLE_GAIN`, `WEIGHT_GAIN`, `FAT_LOSS`, `WEIGHT_LOSS`), mục tiêu chính (`fitnessGoal`), cân nặng đích (`targetWeightKg` khi tăng/giảm cân), trình độ (`fitnessLevel`: `BEGINNER`, `INTERMEDIATE`, `ADVANCED`), mức độ hoạt động (`activityLevel`: `SEDENTARY`, `LIGHTLY_ACTIVE`, `MODERATELY_ACTIVE`, `VERY_ACTIVE`), tần suất (`workoutDaysPerWeek`), thời lượng tối đa (`maxSessionMinutes`), thiết bị (`availableEquipment`), nhóm cơ ưu tiên (`targetMuscleGroups`) và hạn chế vận động (`injuryConstraints`, `mobilityLimitNotes`). **Dữ liệu dinh dưỡng:** `dietaryPreference` (`OMNIVORE`, `VEGETARIAN`, `VEGAN`), `foodAllergies`, `excludedFoods`, `mealsPerDay`.
...
- **Quản lý gói dịch vụ phòng tập (Membership Management):**
  - *Mô tả:* Quản trị viên (Admin) thực hiện CRUD danh mục các gói tập. Hội viên gửi yêu cầu đăng ký gói tập, hệ thống tạo Subscription ở trạng thái **PENDING**; Admin xác nhận mô phỏng tại Dashboard để chuyển sang **ACTIVE**. Khi gia hạn một gói đang ACTIVE: `newEndDate = currentEndDate + packageDuration`. Vòng đời trạng thái: `PENDING → ACTIVE → EXPIRED` và `PENDING/ACTIVE → CANCELLED`. Một subscription chỉ hợp lệ khi `status = ACTIVE` và `startDate <= currentDate < endDate`; `endDate` là mốc hết hiệu lực không bao gồm ngày đó (exclusive). Quyền sử dụng tính năng cao cấp luôn được kiểm tra động theo điều kiện này, không phụ thuộc việc Scheduled Job đã kịp đổi trạng thái sang `EXPIRED` hay chưa. **Lưu ý:** Hết hạn subscription chỉ chặn việc tạo recommendation AI, kích hoạt giáo án và ghi workout log mới; user vẫn đăng nhập, xem dữ liệu lịch sử và gửi yêu cầu gia hạn bình thường. Admin không được khóa tài khoản người dùng chỉ vì hội viên hết hạn gói tập.
...
- **Thư viện bài tập nền tảng (Exercise Library):**
  - *Mô tả:* Admin quản trị danh mục bài tập gồm `name`, nhóm cơ chính (`primaryMuscleGroup`), nhóm cơ phụ (`secondaryMuscleGroups`), kiểu chuyển động (`movementPattern`), vùng cơ thể tác động (`targetBodyRegions`), thiết bị bắt buộc (`equipmentRequired`), độ khó (`difficultyLevel`), tag chống chỉ định (`contraindicationTags`), hướng dẫn (`instructionText`) và trạng thái (`isActive`). Thiết lập API hỗ trợ phân trang, tìm kiếm và lọc trên giao diện React.
...
- **Lập giáo án & Ghi nhận nhật ký tập luyện (Workout Tracking):**
  - *Mô tả:* Gọi AI Engine hoặc fallback template để tạo giáo án `DRAFT` dựa trên hồ sơ thể trạng; cho phép Member xem, kích hoạt giáo án, hiển thị theo ngày và ghi nhận kết quả thực tế (`actualSets`, `actualReps`, `weightUsedKg`, `actualRpe`). Tạo/chỉnh sửa thủ công toàn bộ cấu trúc giáo án không thuộc MVP.
...
- **Phân tích tiến độ tập luyện (Progress Analytics):**
  ...
  - *Liên kết kỹ thuật:* Cung cấp nguồn dữ liệu đã cấu trúc hóa dạng chuỗi thời gian để Frontend sử dụng các thư viện biểu đồ vẽ tiến trình trực quan. Với cân nặng, bản ghi sớm nhất là baseline, bản ghi mới nhất là hiện tại và `targetWeightKg` là mục tiêu độc lập; giao diện hiển thị cả mức thay đổi so với ban đầu lẫn khoảng cách còn lại tới mục tiêu. **Estimated 1RM được chuyển sang Should-have; Must-have chỉ vẽ biểu đồ mức tạ và số reps qua thời gian.**
...
- **Phân hệ PT quản lý hội viên:** Cho phép huấn luyện viên cá nhân xem danh sách hội viên được giao, theo dõi hồ sơ thể trạng, giám sát nhật ký tập và tùy chọn góp ý/chỉnh sửa giáo án `DRAFT` sau khi đề xuất đã được cung cấp trực tiếp cho hội viên. PT không phải bước phê duyệt bắt buộc và không làm chậm luồng Member kích hoạt giáo án trong MVP.
