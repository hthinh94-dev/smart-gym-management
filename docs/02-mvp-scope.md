...
- **Hồ sơ thể trạng hội viên (Member Profile):**
  - *Mô tả:* Quản lý thông tin thể chất chi tiết gồm: họ tên, `dateOfBirth`, `gender`, chiều cao (`heightCm`), cân nặng (`weightKg`), mục tiêu (`fitnessGoal`: `BULK`, `CUT`, `MAINTAIN`), trình độ (`fitnessLevel`: `BEGINNER`, `INTERMEDIATE`, `ADVANCED`), mức độ hoạt động (`activityLevel`: `SEDENTARY`, `LIGHTLY_ACTIVE`, `MODERATELY_ACTIVE`, `VERY_ACTIVE`), tần suất (`workoutDaysPerWeek`), thời lượng tối đa (`maxSessionMinutes`), thiết bị (`availableEquipment`), nhóm cơ ưu tiên (`targetMuscleGroups`) và hạn chế vận động (`injuryConstraints`). **Dữ liệu dinh dưỡng:** `dietaryPreference` (`OMNIVORE`, `VEGETARIAN`, `VEGAN`), `foodAllergies`, `excludedFoods`, `mealsPerDay`.
