...
- **Hiện thực hóa kỹ thuật:** JSON Schema của AI không chứa các trường số liệu định lượng. Calculator dùng BMI và Mifflin-St Jeor, hệ số hoạt động `1.2/1.375/1.55/1.725`, điều chỉnh surplus `BULK`/`MUSCLE_GAIN`/`WEIGHT_GAIN` `+300`, deficit `CUT`/`FAT_LOSS`/`WEIGHT_LOSS` `-500`, còn `MAINTAIN` `+0`; mục tiêu chính `fitnessGoal` quyết định nhánh tính khi hội viên chọn hai mục tiêu. Protein cố định `2.2g/kg`, Fat cố định `25% calories` và Carb nhận phần calories còn lại. Kết quả làm tròn `HALF_UP` đến 2 chữ số. Sau khi AI response hợp lệ, `RecommendationService` ghép `calculatedTargets` do Backend tạo vào response; payload AI có trường ngoài schema bị từ chối và kích hoạt luồng retry/fallback.
...
- **Hiển thị tiến độ:** Frontend sắp xếp lịch sử theo `recordDate` tăng dần và lấy bản ghi đầu tiên làm cân nặng ban đầu. Mọi thông báo tăng/giảm cân và dòng chi tiết trong bảng lịch sử đều so sánh với baseline này, không so sánh với bản ghi liền trước hoặc nhầm với cân nặng mục tiêu.
