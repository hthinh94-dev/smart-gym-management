...
describe("memberBodyProgressApi", () => {
    ...
    it("maps API validation errors", async () => { vi.spyOn(httpClient, "post").mockRejectedValue(Object.assign(new Error("invalid"), { isAxiosError: true, response: { status: 400, data: { success: false, errorCode: "VAL-001", message: "invalid", details: { field: "weightKg" } } } })); await expect(upsertMemberBodyProgress({ recordDate: item.recordDate, weightKg: 0 })).rejects.toMatchObject<Partial<BodyProgressApiError>>({ errorCode: "VAL-001" }); });
